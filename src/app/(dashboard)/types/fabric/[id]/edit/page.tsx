...
export default function EditFabricTypePage() {
    ...
    useEffect(() => {
        const fetchItem = async () => {
            try {
                const response = await fetch(`/api/fabric-types/${params.id}`);
    ...
    const onSubmit = async (data: TypeFormData) => {
        try {
            ...
            if (result.success) {
                toast({ title: "Success", description: "Fabric type updated successfully" });
                router.push("/types?tab=fabric");
            } else {
    ...
    return (
        <div className="flex-1 space-y-6 px-4 md:px-8 pt-6 pb-24 page-enter">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-3xl font-bold tracking-tight font-heading uppercase">Edit Fabric Type</h2>
                        </div>
            ...
            <div className="mx-auto">
                <TypeForm type="fabric" id={params.id as string} initialData={itemData} onSubmit={onSubmit} isLoading={loading} isEdit={true} />
            </div>
        </div>
    );
}
