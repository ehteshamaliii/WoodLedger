...
import {
    ...
    Calendar, FileText, Wallet, BadgeDollarSign, Plus, Pencil, ShoppingCart, AlertTriangle,
} from "lucide-react";
...
export default function PaymentFormPage() {
    ...
    const onSubmit = async (data: PaymentForm) => {
        ...
        if (!isOnline) {
            if (isEdit) {
                await queueAction({
                    entity: 'PAYMENT',
                    action: 'UPDATE',
                    data: { id: params.id, ...data, orderId, updatedAt: new Date() },
                });
            } else {
                await queueAction({
                    entity: 'PAYMENT',
                    action: 'CREATE',
                    data: { id: `temp_${Date.now()}`, ...data, orderId, createdAt: new Date(), date: data.date || new Date().toISOString() },
                });
            }
            toast({ title: "Saved offline", description: `Payment ${isEdit ? 'update' : 'record'} will sync when you're back online.` });
            router.push("/payments");
            return;
        }
        ...
        try {
            setLoading(true);
            const url = isEdit ? `/api/payments/${params.id}` : "/api/payments";
            const method = isEdit ? "PUT" : "POST";
            const res = await fetch(url, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...data, orderId }) });
            const result = await res.json();
            if (result.success) {
                toast({ title: "Success", description: `Payment ${isEdit ? "updated" : "recorded"} successfully` });
                router.push("/payments");
            } else {
                                        ...
                                        {selectedAccount && (
                                            <div className="pt-3 border-t border-border/40">
                                                ...
                                                <p className="text-xs text-muted-foreground font-medium mt-0.5">
                                                    Current balance: Rs. {Number(selectedAccount.balance).toLocaleString()}
                                                </p>
                                                <p className={cn(
                                                    "text-xs font-bold mt-1",
                                                    selectedType === "CREDIT" ? "text-emerald-500" : "text-red-400"
                                                )}>
                                                    After: Rs. {(
